...
export default function MariageModal({ onClose }: { onClose: () => void }) {
  const t = useTranslations('Modals.mariage');
  const tc = useTranslations('Common');
  const demoLink = "https://mariage.neocard.ch";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm overflow-hidden">
      ...
      >
        ...
        <div className="w-full md:w-2/5 bg-slate-900 p-8 pt-12 md:pt-8 flex flex-col shrink-0 relative">
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-10 pointer-events-none" />
          <div className="relative z-10">
            <span className="text-amber-500 text-xs font-bold uppercase tracking-widest mb-2 block">{t('subtitle')}</span>
            ...
            <div className="space-y-4 mb-8">
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Smartphone size={16} className="text-amber-500"/> <span>{tc('features.mobile')}</span></div>
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Globe size={16} className="text-amber-500"/> <span>{tc('features.noApp')}</span></div>
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Shield size={16} className="text-amber-500"/> <span>{tc('features.secure')}</span></div>
            </div>
          </div>
          <div className="mt-auto relative z-10">
             <p className="text-xs text-slate-500 uppercase tracking-widest mb-3 text-center">{tc('viewLiveResult')}</p>
             <a href={demoLink} target="_blank" rel="noopener noreferrer" className="block w-full bg-amber-500 hover:bg-amber-400 text-black font-bold py-4 rounded-lg text-center transition-colors uppercase tracking-widest text-sm flex items-center justify-center gap-2">
        ...
        <div className="w-full md:w-3/5 p-8 bg-black">
           ...
           <div className="grid gap-4">
             ...
             <div className="relative p-5 rounded-lg border border-amber-500/40 bg-amber-900/10 transition-all">
                <span className="absolute -top-2 right-4 bg-amber-500 text-black text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm">{tc('recommended')}</span>
                <div className="flex justify-between items-end mb-3">
                   <h4 className="text-base font-bold text-amber-400">{t('plan2')}</h4>
