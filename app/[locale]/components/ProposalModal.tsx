...
export default function ProposalModal({ onClose }: { onClose: () => void }) {
  const t = useTranslations('Modals.proposal');
  const tc = useTranslations('Common');
  const demoLink = "https://proposal.neocard.ch";
      ...
      >
        ...
        <div className="w-full md:w-2/5 bg-rose-950/50 p-8 pt-12 md:pt-8 flex flex-col shrink-0 relative">
          ...
          <div className="relative z-10">
            <span className="text-rose-400 text-xs font-bold uppercase tracking-widest mb-2 block">{t('subtitle')}</span>
            ...
            <div className="space-y-4 mb-8">
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Smartphone size={16} className="text-rose-400"/> <span>{tc('features.mobile')}</span></div>
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Lock size={16} className="text-rose-400"/> <span>{tc('features.password')}</span></div>
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Heart size={16} className="text-rose-400"/> <span>Effet garanti</span></div>
            </div>
          </div>
          <div className="mt-auto relative z-10">
             <p className="text-xs text-slate-500 uppercase tracking-widest mb-3 text-center">{tc('viewLiveResult')}</p>
             <a href={demoLink} target="_blank" rel="noopener noreferrer" className="block w-full bg-rose-600 hover:bg-rose-500 text-white font-bold py-4 rounded-lg text-center transition-colors uppercase tracking-widest text-sm flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(225,29,72,0.3)]">
           ...
           <div className="grid gap-4">
             <div className="p-5 rounded-lg border border-white/10 bg-white/5 hover:border-white/20 transition-all">
                <div className="flex justify-between items-end mb-3">
                   ...
                   <div className="text-lg font-serif text-white">590 CHF</div>
                </div>
                <ul className="grid grid-cols-1 gap-2">{renderFeatures('plan1_features')}</ul>
             </div>
             <div className="relative p-5 rounded-lg border border-rose-500/40 bg-rose-900/10 transition-all">
                <span className="absolute -top-2 right-4 bg-rose-600 text-white text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm">{tc('wowEffect')}</span>
                <div className="flex justify-between items-end mb-3">
                   <h4 className="text-base font-bold text-rose-400">{t('plan2')}</h4>
                   <div className="text-lg font-serif text-white">890 CHF</div>
                </div>
