...
export default function BarMitzvahModal({ onClose }: { onClose: () => void }) {
  const t = useTranslations('Modals.bar');
  const tc = useTranslations('Common');
  const demoLink = "https://barmitzvah.neocard.ch";
      ...
      >
        ...
        <div className="w-full md:w-2/5 bg-indigo-950/60 p-8 pt-12 md:pt-8 flex flex-col shrink-0 relative">
          ...
          <div className="relative z-10">
            <span className="text-indigo-400 text-xs font-bold uppercase tracking-widest mb-2 block">{t('subtitle')}</span>
            ...
            <div className="space-y-4 mb-8">
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Smartphone size={16} className="text-indigo-400"/> <span>{tc('features.mobile')}</span></div>
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Globe size={16} className="text-indigo-400"/> <span>{tc('features.multilingual')}</span></div>
               <div className="flex items-center gap-3 text-slate-300 text-sm"><Shield size={16} className="text-indigo-400"/> <span>{tc('features.secure')}</span></div>
            </div>
          </div>
          <div className="mt-auto relative z-10">
             <p className="text-xs text-slate-500 uppercase tracking-widest mb-3 text-center">{tc('viewLiveResult')}</p>
             <a href={demoLink} target="_blank" rel="noopener noreferrer" className="block w-full bg-indigo-500 hover:bg-indigo-400 text-white font-bold py-4 rounded-lg text-center transition-colors uppercase tracking-widest text-sm flex items-center justify-center gap-2">
        ...
        <div className="w-full md:w-3/5 p-8 bg-black">
           ...
           <div className="grid gap-4">
             <div className="p-5 rounded-lg border border-white/10 bg-white/5 hover:border-white/20 transition-all">
                ...
                <ul className="grid grid-cols-1 gap-2">{renderFeatures('plan1_features', 'text-indigo-400')}</ul>
             </div>
             <div className="relative p-5 rounded-lg border border-indigo-500/40 bg-indigo-900/10 transition-all">
                <span className="absolute -top-2 right-4 bg-indigo-500 text-white text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm">{tc('recommended')}</span>
                <div className="flex justify-between items-end mb-3">
                   <h4 className="text-base font-bold text-indigo-400">{t('plan2')}</h4>
                   <div className="text-lg font-serif text-white">1'900 CHF</div>
                </div>
                <ul className="grid grid-cols-1 gap-2">{renderFeatures('plan2_features', 'text-indigo-400')}</ul>
             </div>
             <div className="p-5 rounded-lg border border-white/10 bg-white/5 hover:border-white/20 transition-all">
                <div className="flex justify-between items-end mb-3">
                ...
                <ul className="grid grid-cols-1 gap-2">{renderFeatures('plan3_features', 'text-indigo-400')}</ul>
             </div>
           </div>
        </div>
      </motion.div>
